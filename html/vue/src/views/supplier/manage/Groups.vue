<template>
  <div class="nonapp">
    ...
    <!--没有功能的情况结束-->
    <div v-if="!nonApp">
      <div class="but-div"  ><router-link :to="{name:'consumer.app.createapp'}"><a class="button btn-normal"><strong>+</strong>创建功能组</a></router-link></div>
      <table class="table" cellspacing="0" cellpadding="0" border="0">
        <tr>
          <th width="400">功能组名称</th>
          <th width="400">业务线</th>
        ...
        <tr  v-for="item in list.items">
          <td>
           ...
           <a><router-link :to="{name:'supplier.business.querygroup',query:{groupid:item.id}}" >{{item.groupname}}</router-link></a></td>
           <td>{{item.categoryname}}</td>

           <td>{{item.state}}</td>
           <td>
            <a v-on:click="reviewgroups(item.id)">提交审核</a>
      ...
      pages:function(){
        var pag = [];
                 ...
                 methods:{
                  reviewgroups(id){
                    let params = {}
                    this.$http.get($API.supplier.reviewgroups.url,params).then((successData) => {
                      console.log(successData.body)
