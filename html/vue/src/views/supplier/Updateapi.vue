      ...
      <div class="f-input">
        ...
        <table class="table jk-table" cellspacing="0" cellpadding="0" border="0" width="960">
          ...
          <tr v-for="(demo,index) in api.demos">
            ...
            <td><input type="text" class="com-input" style="width:190px;" v-model="demo.desc"/></td>
...
<script>
  export default {
    ...
    /*    self.api.forEach(item=>{
        ...
        let params = {};
        $.extend(params,self.api);
        for(var o in params){
          var item = params[o];
          if(typeof item == 'object'){
           params[o]= JSON.stringify(item)
          }
        }
       console.log(JSON.stringify(params,null,1))
        this.$http.post($API.supplier.publishapi.url,params).then(
          (successData) => {
              // this.dialog = true;
           //      console.log(successData.body.data)
            var self = this;
