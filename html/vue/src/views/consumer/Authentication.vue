<template>
  <div class="authentication">
    <div v-if="!non">
      <div class="acc-f">
        <div class="acc-ch">
          <label class="disabled"><input type="radio" disabled="true" />个人开发者</label>
          <label><input type="radio"  v-model="info.checkstate" v-bind:value="1"/>企业开发者</label>
        </div>
        <ul class="acc-list">
          <li>
  ...
  <div class="control-r" v-if="non">
    <div class="con-ts">
      ...
      <div class="ts-t">
        <h5>提示</h5>
        <p>您未进行认证，无业务权限！</p>
        <router-link :to="{name:'common.personal.consumer.authentication'}"><a class="button  btn-normal" v-on:click="non=false">开发者认证</a></router-link>
      </div>
      <div class="clearfix"></div>
    </div>
  </div>
